...
uint8_t BUFFER_X[BUFFER_LEN];
uint8_t BUFFER_Y[BUFFER_LEN];
uint8_t BUFFER_Z[BUFFER_LEN];
uint32_t idx = 0;
uint32_t max_idx = -1;
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim){
	...
	if( htim == &htim1 ) {
		...
		if (TimerCnt >= 1){
			...
			BUFFER_X[idx] = SPIRead(0x29);
			BUFFER_Y[idx]= SPIRead(0x2B);
			BUFFER_Z[idx] = SPIRead(0x2D);
			idx++;
			if (idx >= BUFFER_LEN){
				READ_DATA = 1;
				idx = 0;
    ...
    /* USER CODE BEGIN 3 */
	  ...
	  if (READ_DATA == 1){
		  for (int i = 0; i<BUFFER_LEN; i++){
			  printf("%i, %i, %i, \r\n", BUFFER_X[i], BUFFER_Y[i], BUFFER_Z[i]);
		  }
		  READ_DATA = 0;
	  }
  }
